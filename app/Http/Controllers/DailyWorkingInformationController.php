 ...
 *          20200902001 カレンダー設定で同じ日が２行出る
 *                      カレンダー設定で打刻や編集で時間設定したあと休日にすると
 *                      打刻データを残しているため
 *          20200904001 日次集計で出勤日で打刻ない場合出力されない
 *                      夜勤用に翌日までの打刻情報を取得しているため
 *                      翌日に打刻あると日付が一致しないため出力対象外となっているため
 *          20200904002 20200904001関連
...
class DailyWorkingInformationController extends Controller
{
    ...
    public function calcMain($params){
        ...
        try {
            ...
            $working_model->setParamAccountidAttribute($login_account_id);
    ...
    public function addDailyCalc($params) {
        ...
        $login_user_code = $authuser->code;
        $login_account_id = $authuser->account_id;
        $work_time->setParamAccountidAttribute($login_account_id);
        // 終了日付で検索（最新の情報として）
        ...
        if(count($work_time_results) > 0){
            ...
            // temporary削除処理
            try{
                $temp_calc_model->setParamAccountidAttribute($login_account_id);
                $temp_calc_model->delTempCalcWorkingtime();
                $temp_working_model->setParamAccountidAttribute($login_account_id);
    ...
    private function calcWorkingTimeDate($params){
        ...
        $attendance_holiday_his_target_flg = false;     // 休日用出勤打刻があるか   20200902001 追加
        $before_holiday_his_date = null;                // 休日用出勤打刻があるか   20200902001 追加
        $before_holiday_his_department_code = null;     // 休日用出勤打刻があるか   20200902001 追加
        $before_holiday_his_user_code = null;           // 休日用出勤打刻があるか   20200902001 追加
        $attendance_holiday_000001_flg = false;         // 休日用出勤打刻があるか   20200902001 追加
        $attendance_holiday_000001_user_code = null;            // 休日用出勤打刻があるか   20200902001 追加
        $attendance_holiday_000001_department_code = null;      // 休日用出勤打刻があるか   20200902001 追加
        $attendance_holiday_000001_record_date = null;    // 休日用出勤打刻があるか   20200902001 追加
        $attendance_holiday_000001_created_at = null;     // 休日用出勤打刻があるか   20200902001 追加
        $attendance_holiday_000001_updated_at = null;     // 休日用出勤打刻があるか   20200902001 追加
        $target_date_out_temp_flg = false;       // 当日分をtempに出力した場合true   20200902001 追加
        ...
        foreach ($worktimes as $result) {
            ...
            Log::debug('   ＊＊＊＊＊＊＊　アカウントID ＊＊＊＊＊＊＊＊＊  $account_id = '.$account_id);
            ...
            Log::debug('         休日用出勤打刻があるか   attendance_holiday_his_target_flg    = '.$attendance_holiday_his_target_flg);
            ...
            $wk_record_time = date_format(new Carbon($result->record_datetime), 'His');
            Log::debug('         休日用出勤打刻設定前   $wk_record_time    = '.$wk_record_time);
            Log::debug('         skip?　is_dup  = '.$is_dup);
            $work_time_sub_model = new WorkTime();
                ...
                if ($result->record_datetime != null && $result->record_datetime != "") {
                    if ($wk_record_time == "000001") {
                        ...
                        if ($result->user_holiday_kubun == null || $result->user_holiday_kubun == 0) {
                            $is_dup = false;
                            Log::debug('         休日用出勤打刻設定前   $is_dup = false ');
                            if ($attendance_holiday_000001_user_code == null) { $attendance_holiday_000001_user_code = $result->user_code; }
                            if ($attendance_holiday_000001_department_code == null) { $attendance_holiday_000001_department_code = $result->department_code; }
                            if ($attendance_holiday_000001_record_date == null) { $attendance_holiday_000001_department_code = $result->record_date; }
                            // 休日分 work_times update 1.user_code 2.department_code 3.record_time
                            Log::debug('         休日用出勤打刻設定前   $result->user_code = '.$result->user_code);
                            Log::debug('         休日用出勤打刻設定前   $result->record_datetime = '.$result->record_datetime);
                            $work_time_sub_model->setParamUsercodeAttribute($result->user_code);
                            $work_time_sub_model->setParamRecordtimeAttribute($result->record_datetime);
                            $work_time_sub_model->delWorkTimeByHolidayCancel();
                            //     20200905 00:00:01  delete 0 a
                            //     20200905 08:30:00  delete 0 b
                            //     20200905 17:30:00  delete 0 b
                            //     if update_at_a < update_at_b   update_at_b  a = delete 1
                            //     else if create_at_a < create_at_b  create_at_b  a = delete 1
                            //     else  b = delete 1
                            $attendance_holiday_000001_created_at = $result->created_at;
                            $attendance_holiday_000001_updated_at = $result->updated_at;
                            $attendance_holiday_000001_flg = true;
                            Log::debug('         休日用出勤打刻設定前   $attendance_holiday_000001_flg    = '.$attendance_holiday_000001_flg);
                            Log::debug('         休日用出勤打刻設定前   $result->created_at    = '.$result->created_at);
                            Log::debug('         休日用出勤打刻設定前   $result->updated_at    = '.$result->updated_at);
                        }
                    }
                    Log::debug('         休日用出勤打刻設定前   $attendance_holiday_000001_user_code    = '.$attendance_holiday_000001_user_code);
                    Log::debug('         休日用出勤打刻設定前   $$result->user_code    = '.$result->user_code);
                    Log::debug('         休日用出勤打刻設定前   $attendance_holiday_000001_record_date    = '.$attendance_holiday_000001_record_date);
                    Log::debug('         休日用出勤打刻設定前   $result->record_date    = '.$result->record_date);
                    if ($attendance_holiday_000001_user_code == $result->user_code &&
                        $attendance_holiday_000001_record_date == $result->record_date) {
                        if ($attendance_holiday_000001_flg) {
                            if ($attendance_holiday_000001_updated_at != null) {
                                if ($result->updated_at != null) {
                                    if ($attendance_holiday_000001_updated_at > $result->updated_at) {
                                        // work_times update 1.user_code 2.department_code 3.record_time
                                        $work_time_sub_model->setParamUsercodeAttribute($result->user_code);
                                        $work_time_sub_model->setParamRecordtimeAttribute($result->record_datetime);
                                        $work_time_sub_model->delWorkTimeByHolidayCancel();
                                        $is_dup = false;
                                    ...
                                    if ($result->created_at != null) {
                                        if ($attendance_holiday_000001_updated_at > $result->created_at) {
                                            // work_times update 1.user_code 2.department_code 3.record_time
                                            $work_time_sub_model->setParamUsercodeAttribute($result->user_code);
                                            $work_time_sub_model->setParamRecordtimeAttribute($result->record_datetime);
                                            $work_time_sub_model->delWorkTimeByHolidayCancel();
                            ...
                            } else if ($attendance_holiday_000001_created_at != null) {
                                if ($result->updated_at != null) {
                                    if ($attendance_holiday_000001_created_at > $result->updated_at) {
                                        // work_times update 1.user_code 2.department_code 3.record_time
                                        $work_time_sub_model->setParamUsercodeAttribute($result->user_code);
                                        $work_time_sub_model->setParamRecordtimeAttribute($result->record_datetime);
                                        $work_time_sub_model->delWorkTimeByHolidayCancel();
                                    ...
                                    if ($result->created_at != null) {
                                        if ($attendance_holiday_000001_created_at > $result->created_at) {
                                            // work_times update 1.user_code 2.department_code 3.record_time
                                            $work_time_sub_model->setParamUsercodeAttribute($result->user_code);
                                            $work_time_sub_model->setParamRecordtimeAttribute($result->record_datetime);
                                            $work_time_sub_model->delWorkTimeByHolidayCancel();
                        ...
                        $attendance_holiday_000001_user_code = $result->user_code;
                        $attendance_holiday_000001_department_code = $result->department_code;
                        $attendance_holiday_000001_record_date = $result->record_date;
                        $attendance_holiday_000001_flg = false;
                    }
                }
            }
            Log::debug('         休日用出勤打刻設定後   $attendance_holiday_000001_flg    = '.$attendance_holiday_000001_flg);
            Log::debug('         休日用出勤打刻設定後   $is_dup    = '.$is_dup);
            // -------- 20200904001 add start -----------------
                ...
                // if ($attendance_target_flg) {
                    ...
                    //     $result->mode != Config::get('const.C005.emergency_time')) {
                        ...
                        // if ($result->working_timetable_from_time > $result->working_timetable_to_time) {
                            Log::debug('         休日用出勤打刻設定前   $target_date_ymd    = '.$target_date_ymd);
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_date    = '.$before_holiday_date);
                            Log::debug('         休日用出勤打刻設定前   $result->department_code    = '.$result->department_code);
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_department_code    = '.$before_holiday_department_code);
                            Log::debug('         休日用出勤打刻設定前   $result->user_code    = '.$result->user_code);
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_user_code    = '.$before_holiday_user_code);
                            Log::debug('         休日用出勤打刻設定前   $attendance_holiday_his_target_flg    = '.$attendance_holiday_his_target_flg);
                            Log::debug('         休日用出勤打刻設定前   $result->holiday_working_date    = '.$result->holiday_working_date);
                            Log::debug('         休日用出勤打刻設定前   $result->user_holiday_description    = '.$result->user_holiday_description);
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_his_date    = '.$before_holiday_his_date);
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_his_department_code    = '.$before_holiday_his_department_code);
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_his_user_code    = '.$before_holiday_his_user_code);
                            // if ($before_holiday_his_date == null) {$before_holiday_his_date = $target_date_ymd;}
                            // if ($before_holiday_his_department_code == null) {$before_holiday_his_department_code = $result->department_code;}
                            // if ($before_holiday_his_user_code == null) {$before_holiday_his_user_code = $result->user_code;}
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_his_date    = '.$before_holiday_his_date);
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_his_department_code    = '.$before_holiday_his_department_code);
                            Log::debug('         休日用出勤打刻設定前   $before_holiday_his_user_code    = '.$before_holiday_his_user_code);
                            if ($target_date_ymd != $before_holiday_his_date) {
                                Log::debug('         休日用出勤打刻設定前   $target_date_ymd != $before_holiday_his_date then');
                            }
                            if ($result->department_code != $before_holiday_his_department_code) {
                                Log::debug('         休日用出勤打刻設定前   $result->department_code != $before_holiday_his_department_code then');
                            }
                            if ($result->user_code != $before_holiday_his_user_code) {
                                Log::debug('         休日用出勤打刻設定前   $result->user_code != $before_holiday_his_user_code then');
                            }
                            if ($target_date_ymd != $before_holiday_his_date ||
                                $result->department_code != $before_holiday_his_department_code ||
                                $result->user_code != $before_holiday_his_user_code) {
                                $attendance_holiday_his_target_flg = false;
                                $before_holiday_his_date = $target_date_ymd;
                                $before_holiday_his_department_code = $result->department_code;
                                $before_holiday_his_user_code = $result->user_code;
                                Log::debug('         休日用出勤打刻設定前 break = '.$attendance_holiday_his_target_flg);
                            }
                            if ($result->holiday_working_date != null && $result->holiday_working_date != "") {
                                if ($result->user_holiday_description == Config::get('const.C013_DESC_VALUE.target_calc_time') ||
                                    ...
                                    Log::debug('         user_holiday_description then 1');
                                    Log::debug('         user_holiday_description then 1');
                                    if ($target_date_ymd == $result->holiday_working_date) {
                                        Log::debug('         target_date_ymd then 1');
                                        if (!$attendance_holiday_his_target_flg) {
                                            ...
                                            Log::debug('        休日用出勤打刻フラグをtrue ');
                                            $attendance_holiday_his_target_flg = true;
                    ...
                    // }
                // }
                Log::debug('         休日用出勤打刻設定後   attendance_holiday_his_target_flg    = '.$attendance_holiday_his_target_flg);
                Log::debug('        $is_today_holiday = '.$is_today_holiday);
                if ($is_today_holiday) {
                    // -------- 20200904001 add start -----------------
                    Log::debug('         target_date_out_temp_flg設定用   target_date_ymd    = '.$target_date_ymd);
                    Log::debug('         target_date_out_temp_flg設定用   before_date    = '.$before_date);
                    Log::debug('         target_date_out_temp_flg設定用   $result->user_code    = '.$result->user_code);
                    Log::debug('         target_date_out_temp_flg設定用   before_user_code    = '.$before_user_code);
                    if ($result->user_code != $before_user_code) {
                        $target_date_out_temp_flg = false;
                    }
                    // 打刻データ配列の設定
                    // if ($result->record_date != $target_date_ymd) {
                    //     Log::debug('        $result->record_datetime をnullに変更 = '.$is_today_holiday);
                    //     $result->record_datetime = null;
                    // }
                    // -------- 20200904001 add end -----------------
                    if ($result->record_datetime != null && $result->mode != null && $result->mode != '') {
                        ...
                        if ($chk_setting == 0)  {
                            ...
                            if ($target_flg == true) {
                                ...
                                if ($current_date == $before_date &&
                                    ...
                                    $before_out_flg = true;
                                    $target_date_out_temp_flg = true;       // 当日分をtempに出力した場合true   20200902001 追加
                                ...
                                // target_date_ymdに打刻ない場合は出勤日の場合tempに出力しなければならない
                                // 打刻ないデータはtempに出力
                                // 20200414 add start
                                Log::debug('        打刻ないデータ  当日の有効打刻なし1');
                                Log::debug('        打刻ないデータ　$before_department_code = '.$before_department_code);
                                Log::debug('        打刻ないデータ　$result->department_code = '.$result->department_code);
                                Log::debug('        打刻ないデータ　$before_user_code = '.$before_user_code);
                                Log::debug('        打刻ないデータ　$result->user_code = '.$result->user_code);
                                ...
                                $temp_out_flg = true;
		                        $user_holiday_kubun = 0;
		                        $holiday_holiday_kubun = 0;
		                        if ($result->user_working_date == $target_date_ymd) {
		                            $user_holiday_kubun = $result->user_holiday_kubun;
		                            if ($result->holiday_holiday_kubun != null) {
		                                $holiday_holiday_kubun = $result->holiday_holiday_kubun;
		                            }
		                        }
                                // 当日分出力済みか？ 済み:$target_date_out_temp_flg=true
                                if ($target_date_out_temp_flg) {
                                    $temp_out_flg = false;
                                } else {
                                    // if (($result->business_kubun != (int)Config::get('const.C007.basic')) || 
                                    //     ($result->user_holiday_kubun != 0)) {
                                    //     $temp_out_flg = false;
                                    // } else {
                                        if ($result->user_working_date == $target_date_ymd) {
                                            if (($wk_record_time != "000001")  && 
                                                ($result->record_datetime != null && $result->record_datetime != "")) {
                                                if ($holiday_holiday_kubun == 0) {
                                                    $temp_out_flg = false;
                                                }
                                            }
                                        }
                                ...
                                if ($temp_out_flg) {
                                    ...
                                    Log::debug('        データ break $result->holiday_holiday_kubun= '.$result->holiday_holiday_kubun);
                                    ...
                                    if ($holiday_holiday_kubun >= (int)Config::get('const.C013.paid_holiday')) {
                                        // 当日の休暇である場合
                                        if ($result->user_working_date == $target_date_ymd) {
                                            $temp_out_flg1 = true;
                                        }
                                    ...
                                    $before_department_code = $result->department_code;
                            ...
                            try{
                                Log::debug('    設定データ誤りの場合　打刻ないデータ  当日の有効打刻なし2');
                                Log::debug('    temp_calc_workingtimesの登録開始');
                        ...
                        // 打刻ないデータはtempに出力
                        // 20200414 add start
                        Log::debug('        打刻ないデータ  当日の有効打刻なし3');
                        Log::debug('        打刻ないデータ　$before_department_code = '.$before_department_code);
                        Log::debug('        打刻ないデータ　$result->department_code = '.$result->department_code);
                        Log::debug('        打刻ないデータ　$before_user_code = '.$before_user_code);
                        Log::debug('        打刻ないデータ　$before_user_code = '.$result->user_code);
                        Log::debug('        打刻ないデータ　$before_holiday_date = '.$before_holiday_date);
                        Log::debug('        打刻ないデータ　$result->user_working_date = '.$result->user_working_date);
                        Log::debug('        打刻ないデータ　$result->holiday_working_date = '.$result->holiday_working_date);
                        Log::debug('        打刻ないデータ　$result->business_kubun = '.$result->business_kubun);
                        Log::debug('        打刻ないデータ　$target_date_out_temp_flg = '.$target_date_out_temp_flg);
                        Log::debug('        打刻ないデータ　$result->user_holiday_kubun = '.$result->user_holiday_kubun);
                        Log::debug('        打刻ないデータ　$result->holiday_holiday_kubun = '.$result->holiday_holiday_kubun);
    ...
    private function setNoInputTimePtn($params)
    {
        ...
        Log::debug('            setNoInputTimePtn $params[ptn] = '.$params['ptn']);
        // パラメータ設定
        $ptn = $params['ptn'];
    ...
    private function insTempCalcItem($params)
    {
        ...
        $temp_calc_model = new TempCalcWorkingTime();
        $temp_calc_model->setAccountidAttribute($account_id);
        Log::debug('                       insTempCalcItem account_id = '.$account_id);
        ...
        // target_dateのデータがなく、翌日以降（=$result->user_working_date）のデータがある場合は
        // $resultの内容は翌日以降のデータである。
        // 影響のある項目はnull設定する。
        $weekday_kubun = null;
        $weekday_kubun_name = null;
        $dt = new Carbon($target_date);
        if ($dt->isSunday()) {
            $weekday_kubun = Config::get('const.C006.sun');
            $weekday_kubun_name = "日曜日";
        }
        if ($dt->isMonday()) {
            $weekday_kubun = Config::get('const.C006.mon');
            $weekday_kubun_name = "月曜日";
        }
        if ($dt->isTuesday()) {
            $weekday_kubun = Config::get('const.C006.tue');
            $weekday_kubun_name = "火曜日";
        }
        if ($dt->isWednesday()) {
            $weekday_kubun = Config::get('const.C006.wed');
            $weekday_kubun_name = "水曜日";
        }
        if ($dt->isThursday()) {
            $weekday_kubun = Config::get('const.C006.thu');
            $weekday_kubun_name = "木曜日";
        }
        if ($dt->isFriday()) {
            $weekday_kubun = Config::get('const.C006.fri');
            $weekday_kubun_name = "金曜日";
        }
        if ($dt->isSaturday()) {
            $weekday_kubun = Config::get('const.C006.sat');
            $weekday_kubun_name = "土曜日";
        }
        $shift_no = null;
        $user_holiday_kubun = 0;
        $user_holiday_name = null;
        if ($target_date == $result->user_working_date) {
            $shift_no = $result->shift_no;
            $user_holiday_kubun = $result->user_holiday_kubun;
            $user_holiday_name = $result->user_holiday_name;
        }
        Log::debug('        insTempCalcItem $user_holiday_kubun =  '.$user_holiday_kubun);
        ...
        if (isset($shift_no)) {
            ...
            $temp_calc_model->setShiftnoAttribute(null);
        }
        ...
        $temp_calc_model->setWeekdaykubunAttribute($weekday_kubun);
        $temp_calc_model->setWeekdaynameAttribute($weekday_kubun_name);
        ...
        if (isset($user_holiday_kubun)) {
            $temp_calc_model->setHolidaykubunAttribute($user_holiday_kubun);
        } else {
            $temp_calc_model->setHolidaykubunAttribute(0);
        }
        if (isset($user_holiday_name)) {
            $temp_calc_model->setHolidaynameAttribute($user_holiday_name);
        } else {
        ...
        for($i=0;$i<count($this->array_calc_mode);$i++){
            ...
            Log::debug('        用途フリー項目 $user_holiday_kubun =  '.$user_holiday_kubun);
            ...
            if (isset($result->use_free_item)) {
                if (substr($result->use_free_item, Config::get('const.USEFREEITEM.day_holiday'), 1) == "0") {
                    if ($user_holiday_kubun != null && $user_holiday_kubun != 0) {
                        $set_chk = "1";
                    } else {
                        $set_chk = "2";
                    }
            ...
            Log::debug('                       insTempCalcItem holiday_kubun = '.$user_holiday_kubun);
            ...
            if ($set_chk == "1") {
                ...
                Log::debug('                       insTempCalcItem なし = '.$user_holiday_kubun);
            ...
            $current_calc = 0;
            if ($target_date == $result->user_working_date) {
                $current_calc = $this->array_calc_calc[$i];
            } else {
                $current_calc = 1;
            }
            $temp_calc_model->setCurrentcalcAttribute($current_calc);
    ...
    private function calcTempWorkingTimeDate($timetables, $target_date){
        ...
        $temp_calc_model = new TempCalcWorkingTime();
        $temp_calc_model->setParamAccountidAttribute($login_account_id);
        $temp_calc_model->getTempCalcWorkingtime();
        ...
        foreach ($worktimes as $result) {
            ...
            if ($current_date == $before_date &&
                ...
                $dtNow = new Carbon();
                Log::debug('        temp_working_time_dates 勤務状態判定  $result->current_calc = '.$result->current_calc);
                Log::debug('        temp_working_time_dates 勤務状態判定  $result->record_datetime = '.$result->record_datetime);
                Log::debug('        temp_working_time_dates 勤務状態判定  $dtNow = '.$dtNow);
                Log::debug('        temp_working_time_dates 勤務状態判定  $result->working_status = '.$result->working_status);
                if ($result->current_calc == '1' && $result->record_datetime < $dtNow) {    // 打刻時刻 < 現在時刻
                    $working_status = $result->working_status;
                }
                Log::debug('        temp_working_time_dates 勤務状態判定  $working_status = '.$working_status);
                // 労働時間の計算
                if ($result->current_calc == '1') {     // 当日分である場合
                    ...
                    if (!$set_calcTimes_flg && !$mode_chk) {
                        ...
                        if ((!isset($result->use_free_item)) ||
                            ...
                            $before_result = $result;
                            $this->not_employment_working = 0;
                            $working_status = $result->working_status;   // 20200904002
                            Log::debug('        勤務状態　 $working_statusを$result->working_status設定1 = '.$working_status);
                            // $working_status = 0;
                            Log::debug('        勤務状態　 $working_statusを0設定1 = '.$working_status);
                            $note = '';
                            $late = '';
                            $leave_early = '';
                            $to_be_confirmed = '';
                    ...
                    $this->not_employment_working = 0;
                    $working_status = $result->working_status;   // 20200904002
                    Log::debug('        勤務状態　 $working_statusを$result->working_status設定2 = '.$working_status);
                    // $working_status = 0;
                    Log::debug('        勤務状態　 $working_statusを0設定2 = '.$working_status);
                    $note = '';
                    $late = '';
                    $leave_early = '';
                    $to_be_confirmed = '';
                ...
                try{
                    ...
                    $this->not_employment_working = 0;
                    $working_status = $result->working_status;   // 20200904002
                    Log::debug('        勤務状態　 $working_statusを$result->working_status設定3 = '.$working_status);
                    // $working_status = 0;
                    Log::debug('        勤務状態　 $working_statusを0設定3 = '.$working_status);
                    $note = '';
                    $late = '';
                    $leave_early = '';
                    $to_be_confirmed = '';
                ...
                if ($result->current_calc == '1' && !$mode_chk) {
                    ...
                    if ((!isset($result->use_free_item)) ||
                        ...
                        $this->not_employment_working = 0;
                        $working_status = $result->working_status;   // 20200904002
                        Log::debug('        勤務状態　 $working_statusを$result->working_status設定4 = '.$working_status);
                        // $working_status = 0;
                        Log::debug('        勤務状態　 $working_statusを0設定4 = '.$working_status);
                        $note = '';
                        $late = '';
                        $leave_early = '';
                        $to_be_confirmed = '';
                ...
                try{
                    ...
                    $this->not_employment_working = 0;
                    $working_status = $result->working_status;   // 20200904002
                    Log::debug('        勤務状態　 $working_statusを$result->working_status設定5 = '.$working_status);
                    // $working_status = 0;
                    Log::debug('        勤務状態　 $working_statusを0設定5 = '.$working_status);
                    $note = '';
                    $late = '';
                    $leave_early = '';
                    $to_be_confirmed = '';
                ...
                if ($result->current_calc == '1' && !$mode_chk) {
                    ...
                    if ((!isset($result->use_free_item)) ||
                        ...
                        $this->not_employment_working = 0;
                        $working_status = $result->working_status;   // 20200904002
                        Log::debug('        勤務状態　 $working_statusを$result->working_status設定6 = '.$working_status);
                        // $working_status = 0;
                        Log::debug('        勤務状態　 $working_statusを0設定6 = '.$working_status);
                        $note = '';
                        $late = '';
                        $leave_early = '';
                        $to_be_confirmed = '';
                ...
                try{
                    ...
                    $this->not_employment_working = 0;
                    $working_status = $result->working_status;   // 20200904002
                    Log::debug('        勤務状態　 $working_statusを$result->working_status設定7 = '.$working_status);
                    // $working_status = 0;
                    Log::debug('        勤務状態　 $working_statusを0設定7 = '.$working_status);
                    $note = '';
                    $late = '';
                    $leave_early = '';
                    $to_be_confirmed = '';
                ...
                if ($result->current_calc == '1' && !$mode_chk) {
                    ...
                    if ((!isset($result->use_free_item)) ||
                        ...
                        $this->not_employment_working = 0;
                        $working_status = $result->working_status;   // 20200904002
                        Log::debug('        勤務状態　 $working_statusを$result->working_status設定8 = '.$working_status);
                        // $working_status = 0;
                        Log::debug('        勤務状態　 $working_statusを0設定8 = '.$working_status);
                        $note = '';
                        $late = '';
                        $leave_early = '';
                        $to_be_confirmed = '';
        ...
        if ($calc_nobreak_cnt > 0 && $current_result->current_calc != 0) {
            if (count($array_add_attendance_time) > 0 ||
                ...
                try{
                    if ($current_result->current_calc == '1' && $current_result->record_datetime < $dtNow) {    // 打刻時刻 < 現在時刻
                        $working_status = $current_result->working_status;
                    }
    ...
    private function addTempWorkingTimeDate($params)
    {
        ...
        try{
            ...
            $apicommon = new ApiCommonController();
            $temp_working_model = new TempWorkingTimeDate();
            $temp_working_model->setAccountidAttribute($login_account_id);
            ...
            if ($target_result->business_kubun == Config::get('const.C007.basic')) {
                ...
                if ($total_time_stamp < 0) { $total_time_stamp = 0; }       // 20200904
            ...
            $temp_working_model->setWorkingtimetablenoAttribute($target_result->working_timetable_no);
            Log::debug('insertTempWorkingTimeDate $target_result->working_timetable_no = '.$target_result->working_timetable_no);
            Log::debug('insertTempWorkingTimeDate working_status = '.$working_status);
    ...
    private function isCurrentDateCalc($params)
    {
        ...
        Log::debug('         isCurrentDateCalc モード $result->mode = '.$result->mode);
        Log::debug('         isCurrentDateCalc $result->record_datetime = '.$result->record_datetime);
