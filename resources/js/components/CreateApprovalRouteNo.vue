    ...
    <div class="row justify-content-between">
      ...
      <div class="col-md pt-3">
        <div class="card shadow-pl">
          ...
          <daily-working-information-panel-header
            v-bind:header-text1="'承認者ルートを設定する'"
            v-bind:header-text2="'承認者ルートの登録や変更ができます'"
          ></daily-working-information-panel-header>
          <!-- /.panel header -->
                ...
                <div class="input-group">
                  <div class="input-group-prepend">
                    <span
                      ...
                      id="basic-addon1"
                    >承認者ルート選択<span class="color-red">[必須]</span></span>
                  </div>
                  <select-approvalrouteList v-if="showlist"
                    ref="selecttimetablelist"
                    v-bind:blank-data="false"
                    v-bind:placeholder-data="'承認者ルートを選択すると編集モードになります'"
      ...
      <div class="col-md-12 pt-3" v-if="selectMode=='NEW'">
        <div class="card shadow-pl">
          <!-- panel header -->
          <daily-working-information-panel-header
            v-bind:header-text1="'◆承認者ルート登録'"
            v-bind:header-text2="'承認者ルート名は一覧選択などでわかりやすい名称を入力します'"
          ></daily-working-information-panel-header>
          <!-- /.panel header -->
          <!-- ----------- 新規入力部 START ---------------- -->
          <!-- panel contents -->
                ...
                <div class="input-group">
                  <div class="input-group-prepend">
                    <span
                      class="input-group-text font-size-sm line-height-xs label-width-180"
                      id="basic-addon1"
                    >承認者ルート名<span class="color-red">[必須]</span></span>
                  </div>
                  <input
                    type="text"
                    class="form-control"
...
if (this.messagevalidatesNew.length > 0) {
    ...
    FixDetail(index, eventtext) {
      ...
      this.postRequest("/create_time_table/fix", arrayParams)
        ...
        .catch(reason => {
          messages.push("承認者ルート" + eventtext + "に失敗しました");
          this.htmlMessageSwal("エラー", messages, "error", true, false);
        });
    },
    // 承認者ルート削除処理（明細）
    ...
    putThenHead(response, eventtext) {
      ...
      var res = response.data;
      if (res.result) {
        this.$toasted.show("承認者ルートを" + eventtext + "しました");
    ...
    serverCatch(eventtext) {
      var messages = [];
      messages.push("承認者ルート情報" + eventtext + "に失敗しました");
      this.htmlMessageSwal("エラー", messages, "error", true, false);
    },
    inputClear() {
      this.details = [];
