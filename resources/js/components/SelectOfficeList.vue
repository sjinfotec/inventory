...
export default {
  ...
  methods: {
    ...
    getThen(response) {
      ...
      console.log('selecOfficeList getThen = res.result' + res.result);
      if (res.result) {
        // 固有処理 START
        console.log('selecOfficeList getThen = res.details' + res.details.length);
        this.itemList = res.details;
        if (this.addNew) {
          this.object = { name: "新規に営業所を登録する", code: "" };
