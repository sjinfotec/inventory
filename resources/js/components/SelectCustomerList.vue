...
export default {
  ...
  methods: {
    ...
    getThen(response) {
      ...
      console.log('selecCustomerList getThen = res.result' + res.result);
      if (res.result) {
        // 固有処理 START
        console.log('selecCustomerList getThen = res.details' + res.details.length);
        this.itemList = res.details;
        if (this.addNew) {
          this.object = { name: "新規に客先を登録する", code: "" };
