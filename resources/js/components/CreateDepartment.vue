...
export default {
  ...
  // マウント時
  mounted() {
    //this.$refs.selectdepartment.getDepartmentList(moment(new Date()).format("YYYYMMDD"), false);
  },
  methods: {
    // ------------------------ バリデーション ------------------------------------
    // バリデーション（新規作成）
    ...
    FixDetail(kbnname, index) {
      ...
      this.postRequest("/create_department/fix", arrayParams)
        ...
        .catch(reason => {
          messages.push("部署" + kbnname + "に失敗しました");
          this.messageswal("エラー", messages, "error", true, false, true);
        });
    },
    // 部署削除処理（明細）
    ...
    putThenHead(response, eventtext) {
      ...
      if (res.result) {
        ...
        if (res.messagedata.length > 0) {
          ...
          messages.push("部署" + eventtext + "に失敗しました");
          this.messageswal("エラー", messages, "error", true, false, true);
        }
      }
    },
