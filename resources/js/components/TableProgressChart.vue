            ...
            <table class="table table-striped border-bottom font-size-sm text-nowrap">
              ...
              <tbody>
                <tr v-for="(item,rowIndex) in details" :key="rowIndex">
                  <td class="text-left align-middle mw-rem-8">{{ item['supply_date'] }}</td>
                  <td class="text-left align-middle mw-rem-20">{{ item['customer_name'] }}</td>
                  <td class="text-left align-middle mw-rem-30">{{ item['order_no'] }}</td>
...
export default {
  ...
  methods: {
    ...
    getItem() {
      ...
      this.postRequest("/get_progress_chart", arrayParams)
        .then(response => {
