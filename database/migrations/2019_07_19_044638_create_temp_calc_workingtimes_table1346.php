...
class CreateTempCalcWorkingtimesTable1346 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('temp_calc_workingtimes', function (Blueprint $table) {
            ...
            $table->time('attendance_time')->nullable()->comment('出勤時刻');
            $table->time('leaving_time')->nullable()->comment('退勤時刻');
            $table->unsignedDecimal('weekday_kubun',1,0)->nullable()->comment('曜日区分');
            $table->String('weekday_name')->nullable()->comment('曜日名称');
            $table->unsignedDecimal('leave_kubun',2,0)->nullable()->comment('出勤区分');
            $table->String('leave_name')->nullable()->comment('出勤名称');
